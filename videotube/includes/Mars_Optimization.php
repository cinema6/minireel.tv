		...
		function enable_seo(){
			...
			if( is_single() || is_singular('video') || is_page() ){
				...
				if( has_post_thumbnail( $post->ID ) ){
					...
					$thumb_url = wp_get_attachment_image_src($thumb_id,'590-300', true);
					$meta .= '<meta property="og:image" content="'.$thumb_url[0].'">';
				}
				if( strpos( $post_datas->post_content, '[vc_row]') === false ){
					$meta .= '<meta property="og:description" content="'. wp_trim_words( $post_datas->post_content, apply_filters( 'mars_seo_desc_number' , 50) ) .'">';
				}
