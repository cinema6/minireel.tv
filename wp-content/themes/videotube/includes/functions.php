...
function modify_youtube_embed_url($html, $url, $args) {
	global $videotube;
	if (strpos($url, 'youtube')!==FALSE && $videotube['autoplay'] == 1) {
	...
	function mars_orderblock_videos() {
		...
		$block = '<div class="section-nav"><ul class="sorting"><li class="sort-text">'.__('Sort by:','mars').'</li>';
			foreach ( $sort_array as $key=>$value ){
				...
				if( is_search() ){
					$block .= '<li '.$active.'><a href="?s='.get_query_var('s').'&order='.$key.'">'.$value.'</a></li>';
				}
				else{
...
if ( ! function_exists( 'mars_pagination' ) ){
	...
	function mars_pagination( $query = null ) {
		...
		$links = paginate_links( array(
			...
			'prev_next'    => True,
			'prev_text' => __( '&larr; First', 'mars' ),
			'next_text' => __( 'Last &rarr;', 'mars' ),
		) );
...
if( !function_exists( 'mars_check_latest_version_update' ) ){
	/**
	 * Check the latest version for automatic update.
	 */
	function mars_check_latest_version_update() {
		global $videotube;
		if( !class_exists( 'Envato_WordPress_Theme_Upgrader' ) ){
			include_once('class-envato-wordpress-theme-upgrader.php');
		}
		$username = isset( $videotube['username'] ) ? esc_attr( $videotube['username'] ) : null;
		$api = isset( $videotube['api'] ) ? esc_attr( $videotube['api'] ) : null;

		if( !$username || !$api )
			return;

		$upgrader = new Envato_WordPress_Theme_Upgrader( $username, $api );
	}
	add_action('admin_init', 'mars_check_latest_version_update');
}
